...
updates_dir=/data/colt_updates

if [ ! -f "$1" ]; then
   echo "Usage: $0 ZIP [UNVERIFIED]"
   echo "Push ZIP to $updates_dir and add it to Updater"
   echo
   echo "The name of ZIP is assumed to have ColtOS-VERSION-DATE-TYPE-* as format"
...
# Assume ColtOS-VERSION-DATE-TYPE-*.zip
...
adb shell "killall com.colt.updater 2>/dev/null"
adb shell "sqlite3 /data/data/com.colt.updater/databases/updates.db" \
